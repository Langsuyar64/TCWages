...
namespace TCWages.API.Controllers {
    ...
    public class PhotosController : ControllerBase {
        ...
        public async Task<IActionResult> AddPhotoForUser(int userId, [FromForm]PhotoForCreationDto photoForCreationDto)
        {
            ...
            // if (file.Length > 0)
            // Check if file has been selected and not empty before uploading
            if (file == null || file.Length == 0)
            {
                return BadRequest("No file to upload or send. Please specify file!");
            }
            else
            {
            ...
            {
                var phtoToReturn = _mapper.Map<PhotosForReturnDto>(photo);
                return CreatedAtRoute("GetPhoto", new {id = photo.Id}, phtoToReturn);
            }
